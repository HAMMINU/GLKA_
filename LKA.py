...
class LKA(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=None):
        ...
        # 1) Depthwise conv: 공간적 어텐션 추출 (in_ch → in_ch)
        ...
        # 2) Pointwise conv for attention map (in_ch → in_ch)
        self.pw_attn = nn.Conv2d(self.in_ch, self.out_ch,
    ...
    def forward(self, x):
        ...
        attn = self.dw1(x)
        attn = self.dw2(attn)
        attn = self.pw_attn(attn)           # [B, out_ch, H, W]

        # b) skip connection 준비 (채널 맞추기)
